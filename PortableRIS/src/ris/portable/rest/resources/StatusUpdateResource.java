...
public class StatusUpdateResource {
	...
	public Response doGet(
	        ...
	        //2020.12.09 Add Nishihara@COSMO start
			//受付ボタン処理に呼出ステータスが追加となったため、処理を分けるために現在のステータスを保持
			@QueryParam("orderstatus") String orderstatus,
	        //2020.12.09 Add Nishihara@COSMO end
...
//        return doPost(status_id, ris_id, kensasitu_id, kensakiki_id, mpps, request);
        return doPost(status_id, ris_id, kensasitu_id, kensakiki_id, kanja_nyugai, kensatype_id,
        		zisshisha_id1, zisshisha_id2, zisshisha_id3, zisshisha_name1, zisshisha_name2, zisshisha_name3, orderstatus, mpps, request);
	}
	...
	public Response doPost(
		...
		if (STATUSID_BACK.equals(status_id)) { // 戻る
			...
			//2020.12.09 Mod Nishihara@COSMO start 呼出ステータス時の処理追加
			// ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_UNREGISTERED, Const.STATUS_ISREGISTERED, request, statusDto, 0);
			if(Const.STATUS_ISCALLING.equals(orderstatus)) {
				ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_ISCALLING, Const.STATUS_ISREGISTERED, request, statusDto, 0);
			}
			else {
				ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_UNREGISTERED, Const.STATUS_ISREGISTERED, request, statusDto, 0);
			}
			//2020.12.09 Mod Nishihara@COSMO end 呼出ステータス時の処理追加
		...
		// 2019.08.06 Add Cosmo Start 排他対応
			...
			//2020.12.09 Mod Nishihara@COSMO start 呼出ステータス時の処理追加
			// ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_UNREGISTERED, Const.STATUS_ISREGISTERED, request, statusDto, 1);
			if(Const.STATUS_ISCALLING.equals(orderstatus)) {
				ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_ISCALLING, Const.STATUS_ISREGISTERED, request, statusDto, 1);
			}else {
				ExecuteRegistered(ris_id.get(0).toString(), Const.STATUS_UNREGISTERED, Const.STATUS_ISREGISTERED, request, statusDto, 1);
			}
			//2020.12.09 Mod Nishihara@COSMO end 呼出ステータス時の処理追加
		//2020.09.14 Add Nishihara@COSMO start 複数オーダーの一括処理機能追加 一括受付、一括MWM登録対応
			...
			for(int i=0; i<ris_id.size(); i++) {
				ExecuteLumpRegistered(ris_id.get(i).toString(), Const.STATUS_UNREGISTERED, Const.STATUS_ISREGISTERED, kensakiki_id, request, statusDto, 1);
			}
	...
	private boolean ExecuteRest(
		...
		try{
			...
			try {
                ...
                // 登録情報取得
        		// 実績メイン情報取得
        		exBean = Configuration.GetInstance().GetCoreController().GetRisExecutionInfo(ris_id, 0, risconn);
//        		exBean = Configuration.GetInstance().GetCoreController().GetRisExecutionInfo(ris_id, risconn);

        		// オーダ情報取得
        		orderBean = Configuration.GetInstance().GetCoreController().GetRisOrderInfo(ris_id, 0, risconn);
