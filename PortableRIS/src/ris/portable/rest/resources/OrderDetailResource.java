				...
				if(config.getExclusive()){
					...
					if (checkoutID != null && checkoutID.length() > 0)
					{
						...
						if (checkoutRow != null)
						{
							...
							OrderInfoBean orderBean = Configuration.GetInstance().GetCoreController().GetRisOrderInfo(ris_id, 0, risconn);
//							OrderInfoBean orderBean = Configuration.GetInstance().GetCoreController().GetRisOrderInfo(ris_id, risconn);
				...
				// オーダ詳細情報設定
				dto.setOrder_detail(this.getOrderDetail(config, orderDt, risconn, userInfo));
	 ...
	 * @throws Exception
	 */
	private OrderDetailDto.OrderDetail getOrderDetail(Config config, DataTable dt, Connection conn, UserInfo userInfo) throws Exception {
		...
		for (int i=0; i<dtBuiInfo.getRowCount(); i++) {
			...
			// 受済以上のオーダは実績部位情報の撮影ステータスで判定し、受済未満は実績部位情報がないため【未】とする
			if (Integer.parseInt(dt.getRows().get(0).get("STATUS").toString()) >= Integer.parseInt(Const.STATUS_ISREGISTERED)) {
				if(Const.SATUEISTATUS_FINISH.equals(dtBuiInfo.getRows().get(i).get("SATUEISTATUS").toString())) {
					buiInfo += "【済】";
				}
				else {
					buiInfo += "【未】";
		...
		// アクセッション番号
		parameterReplacerOrderInfo.setAccessionNo(order.getAcno());
